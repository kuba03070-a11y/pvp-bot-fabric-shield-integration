...
public class BotNavigation {
    ...
    private static void moveTowardPos(ServerPlayerEntity bot, Vec3d targetPos, double speed, NavigationState state) {
        ...
        if (inWater) {
            ...
            if (targetFar) {
                bot.setSprinting(false); bot.forwardSpeed = 0.8f; bot.sidewaysSpeed = 0;
                bot.addVelocity(dx * speed * 0.02, 0, dz * speed * 0.02);
                if (state.jumpCooldown <= 0) { bot.jump(); state.jumpCooldown = 8; }
            } else {
                bot.setSprinting(false); bot.forwardSpeed = 0.6f; bot.sidewaysSpeed = 0;
        ...
        if (bot.isOnGround() && state.jumpCooldown <= 0) {
            if (canJumpUp) {
                bot.jump(); if (jumpBoost > 0) bot.addVelocity(0, jumpBoost, 0);
                bot.addVelocity(dx * 0.2, 0, dz * 0.2); state.jumpCooldown = 8;
            } else if (onLadder) {
                bot.jump(); if (jumpBoost > 0) bot.addVelocity(0, jumpBoost, 0); state.jumpCooldown = 5;
            } else if (holeAhead) {
                bot.jump(); if (jumpBoost > 0) bot.addVelocity(0, jumpBoost, 0);
                bot.addVelocity(dx * 0.35, 0.05, dz * 0.35); state.jumpCooldown = 12;
            } else if (isWall && state.avoidTicks <= 0) {
                state.avoidDirection = (Math.random() > 0.5) ? 1 : -1; state.avoidTicks = 25;
                bot.jump(); if (jumpBoost > 0) bot.addVelocity(0, jumpBoost, 0); state.jumpCooldown = 10;
            }
        }

        if (onLadder) {
            bot.addVelocity(0, 0.12, 0); bot.setSprinting(false); bot.forwardSpeed = 1.0f;
            state.lastPosition = botPos; return;
        }
        ...
        if (shouldBhop && bot.isOnGround()) {
            bot.jump(); if (jumpBoost > 0) bot.addVelocity(0, jumpBoost, 0); state.jumpCooldown = bhopCooldown;
        }

        bot.setSprinting(true); bot.forwardSpeed = 1.0f; bot.sidewaysSpeed = 0;
    ...
    public static void lookAt(ServerPlayerEntity bot, Entity target) {
        ...
        bot.setYaw(yaw); bot.setPitch(pitch); bot.setHeadYaw(yaw);
    }
    ...
    public static void lookAway(ServerPlayerEntity bot, Entity target) {
        ...
        float yaw = (float)(Math.atan2(dz, dx) * (180.0 / Math.PI)) - 90.0f;
        bot.setYaw(yaw); bot.setPitch(0); bot.setHeadYaw(yaw);
    }
    ...
    public static void idleWander(ServerPlayerEntity bot) {
        ...
        if (dist > 0.5) {
            dx /= dist; dz /= dist;
            float yaw = (float)(Math.atan2(dz, dx) * (180.0 / Math.PI)) - 90.0f;
            bot.setYaw(yaw); bot.setHeadYaw(yaw); bot.setPitch(0);
            bot.setSprinting(false); bot.forwardSpeed = 0.5f;
            ...
            if (isBlockSolid(world, feetPos) && !isBlockSolid(world, feetPos.up()) &&
                ...
                bot.jump(); state.jumpCooldown = 10;
            }
        } else {
            bot.forwardSpeed = 0; bot.sidewaysSpeed = 0;
        }
        state.lastPosition = botPos;
    }
